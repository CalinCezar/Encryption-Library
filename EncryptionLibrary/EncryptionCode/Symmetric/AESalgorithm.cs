...
namespace EncryptionLibrary.EncryptionCode.Symmetric
{
    ...
    0x17, 0x2B, 0x04, 0x7E, 0xBA, 0x77, 0xD6, 0x26, 0xE1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0C, 0x7D }; //F
        private byte[] roundCoefficient;

        private Random random = new Random();

        private int keySize = 128 / 8;
        private int roundNo = 10;
        ...
        public byte[] Key
        {
            ...
            set
            {
                if (value == null || value.Length * 8 != keySize)
                    return;
                key = value;
                Array.Copy(key, subkeys[0], keySize);
                for (int i = 0; i < roundNo; ++i)
                {
                    Array.Copy(subkeys[i], subkeys[i + 1], keySize);
                    keywordTransform(ref subkeys[i + 1], i);
                }
            }
        }
        ...
        public AESalgorithm(int keySize = 128)
        {
            SetKeySize(keySize);
            Initialize();
        ...
        {
            roundCoefficient = new byte[roundNo];
            for (int i = 0; i < roundNo; i++)
            {
                roundCoefficient[i] = (byte)(1 << i);
            }
        }
        ...
        {
            if (roundno < 0 || roundno > roundNo - 1)
                return;
            byte buf = keyword[0];
        ...
        {
            // Used for Galois multiplication with 2
        ...
        private void encryptBlock(byte[] plaintext, ref byte[] ciphertext)
        {
            ...
            for (int r = 0; r < roundNo; ++r)
            {
                ...
                for (int j = 0; j < AESalgorithm.BLOCK_SIZE; ++j)
        ...
        private void decryptBlock(byte[] ciphertext, ref byte[] plaintext)
        {
            ...
            {
                for (int j = 0; j < BLOCK_SIZE; ++j)
                    plaintext[j] ^= Subkeys[roundNo - r][j];
                if (r > 0)
        ...
        {
            if (plaintext == null || ciphertext == null || ciphertext.Length < plaintext.Length)
                return false;
            ...
            if (extrabytes > 0)
            {
                ...
                for (int i = extrabytes; i < BLOCK_SIZE; ++i)
                    text[i] = 0;
                encryptBlock(text, ref text);
                Array.Copy(text, 0, ciphertext, p, BLOCK_SIZE);
            }
